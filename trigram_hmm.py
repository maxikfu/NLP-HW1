...
#counting for spa and eng words separate
    ...
    return emission_count,spa_emission_count,eng_emission_count,tag_count_dictionary,spa_tag_count,eng_tag_count,transition_count, word_counting, sentence_counter
...
def viterbi(test_data_path, state_graph, tag_count, transition_count, emission_count, word_counting,norm_lambda,spa_emiss,eng_emiss,spa_tag,eng_tag):
...
#choosing dictionary based on language
            ...
            for state in state_graph:
                ...
                for previous_step_state in state_graph:  # computing new value based on previouse step and choosing
                    # maximum at the end
                    max_value_at_this_level = []
                    max_backtrack_at_this_level = []
                    for pre_previous_step_state in state_graph_extended:
                        transition_probability = 0
                        trigram_p_carrot = 0
                        bigram_p_carrot = 0
                        unigram_p_carrot = 0
                        a = 0
                        trigram_tuple = tuple([pre_previous_step_state, previous_step_state, state])
                        bigram_tuple = tuple([previous_step_state, state])
                        if trigram_tuple in transition_variable and bigram_tuple in transition_variable:
                            trigram_p_carrot = transition_variable[trigram_tuple] / transition_variable[tuple([pre_previous_step_state, previous_step_state])]
                        if bigram_tuple in transition_variable and pre_previous_step_state in tag_variable:
                            bigram_p_carrot = transition_variable[bigram_tuple] / tag_variable[previous_step_state]
                        if state in tag_variable:
                            unigram_p_carrot = tag_variable[state] / n
                    ...
                    backtrack_values.append((max(max_backtrack_at_this_level)))
...
emission_count,spa_emiss,eng_emiss,tag_count_dictionary,spa_tag,eng_tag, transition_count, word_counting, sentence_counter = counting_tri_bi_uni_grams(
    'dataset/train.conll')
...
viterbi('dataset/small_dataset.txt', state_graph, tag_count_dictionary, transition_count,emission_count, word_counting, \
        lambda_param, spa_emiss,eng_emiss,spa_tag,eng_tag)
'''
orig_stdout = sys.stdout
